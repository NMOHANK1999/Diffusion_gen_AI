...
\newcommand{\outDate}{Sep. 23, 2024}
\newcommand{\dueDate}{Oct. 07, 2024}
\newcommand{\taNames}{Shreeya, Aravind, Jacob, Katie}
...
\part Suppose we feed a sentence $w_1, \ldots, w_N$ of length $N$ into an \emph{encoder-only} Transformer model, to obtain one output layer embedding $\hv_n \in \Rb^D$ per word $w_n$. We then compute a score vector $\sv_n$ per word $w_n$ as follows:
\begin{align*}
...
\subpart[3] \textbf{Drawing:} Draw the corresponding factor graph for this probability distribution. Your drawing must include exactly $2N$ nodes, one node for each word $w_n$ in the sentence and one for each tag $t_n$. 
    \begin{answer_box}[title=,height=4cm, width=14cm]
    \end{answer_box}
...
\part Suppose we want to define a GAN-inspired model for inpainting grayscale images. Each original image is a matrix $\xv \in (0,1)^{N_h \times N_w}$ with scalars between $0$ and $1$ exclusive. The image is accompanied by a binary pixel mask $\mv \in \{0,1\}^{N_h \times N_w}$, where $m_{ij}=1$ indicates that the pixel should be masked out and $m_{ij}=0$ indicates the pixel should be left intact. Our model is a variant of U-Net that takes in an input image $\xv$ and and a mask $\mv$ and returns a reconstructed image $\xv'$ with the masked pixels filled in by the model, $\xv' = g_{\theta}(\xv, \mv)$. 
You decide to train your model by using two loss functions in combination.

\begin{subparts}

\subpart[1] \textbf{Short answer:} Define a mathematical expression for $\xv'$ that will use the model $g_{\theta}$ and a mask $\mv$ (as defined above) to create an in-filled image, ensuring that the masked-out pixels are never visible to $g_{\theta}$.
...
\begin{parts}
\part [5] Prove that we can break down Equation \eqref{eq:elbo_diffusion} as:
...
\clearpage
\part [2] Can you explain in words what is the effect of the term ${\mathcal{L}_t}$ on the reverse process $p_\theta(\xv_t \mid \xv_{t+1})$ of the diffusion model when we try to maximize the ELBO in Equation~\eqref{eq:elbo_breakdown} and why? When is this term maximized? 
...
\uplevel{\subsection*{Dataset}} The dataset for this homework is the Animal Faces-HQ dataset \href{https://github.com/clovaai/stargan-v2/blob/master/README.md#animal-faces-hq-dataset-afhq}{(AFHQ)}, consisting of 15,000 high-quality images at $512\times512$ resolution. The dataset includes three domains of cat, dog, and wildlife, and in our assignment you \textbf{only need to use cat} images to reduce the computation complexity.
